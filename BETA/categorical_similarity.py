...
def similarity_calc():
    ...
    with open('science_database.txt', 'r') as dataset:
        for line in dataset:
            line = line.rstrip()
            [name, location, department, style, interaction, organism, technique]= line.split('\t')
            data_dict[name] = {}
            ...
            data_dict[name]['organism'] = organism
            data_dict[name]['technique'] = technique

    user_data = {}
    with open('user_science_answers.txt', 'r') as dataset:
        for line in dataset:
            line = line.rstrip()
            [username, location, department, style, interaction, organism, technique] = line.split('\t')
    user_data['location'] = location
    user_data['department'] = department
    user_data['style'] = style
    user_data['interaction'] = interaction
    user_data['organism'] = organism
    user_data['technique'] = technique
